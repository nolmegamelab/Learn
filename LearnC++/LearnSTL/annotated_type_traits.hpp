...
// type_traits는 MPL의 보물창고이다. 난해해 보이지만 
// 컴파일러가 튜링 컴플리트한 순수 함수형 언어를 내장하고 있는 언어는 C++ 밖에 없다. 
// 이 점이 C++을 강력하면서도 사용이 어렵게 만드는 핵심 원인이다. 
// 앱 구현 코드에는 이런 코드가 없도록 하고, 앱용 라이브러리 구현에만 사용한다. 
// 
...
// __make_integer_seq가 실제로 시퀀스를 생성해 준다. 
...
// variadic template은 매우 편리하다. 
// ... _Rest가 비어 있을 경우 여기에 매칭 된다. (type_traits.cpp 코드 참조)
...
// __is_class와 __is_union은 컴파일러 내부 구현으로 보인다. 
...
// odr-used는 definition (인스턴스 생성)이 필요한 사용처가 있다는 뜻이다. 
// constructible 하면서 trivial operation으로만 생성되는 경우이다. 
// Basically this means that a copy or move constructor is trivial if:
// - it is not user-provided, 
// - the class has nothing virtual in it, 
// - and this property holds recursively for all the members of the class and for the base class.
