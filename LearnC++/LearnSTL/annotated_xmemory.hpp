...
// xmemory는 microsoft stl 구현의 기반이 되는 클래스, 함수들을 갖는다. 
...
struct _Tidy_guard { // class with destructor that calls _Tidy
    ...
    ~_Tidy_guard() {
        if (_Target) {
            _Target->_Tidy(); // tidy는 깔끔하게 정돈된 또는 그런 상태로 만들다는 뜻을 갖는다. 
        }
    }
};
...
// C++의 문제는 안전한 부분 집합이 문제가 아니라 이런 암호와 같은 코드가 더 문제이다. 
// [1] declval은 decltype으로 타잎을 추론하고자 하는데 기본 생성자가 없는 경우 T&&의 오브젝트를 만들어 돌려준다. 
// [2] _Keycmp의 () operator는 두 개 타잎의 오브젝트를 얻어 bool로 변환 가능한 값을 돌려준다. 
// [3] noexcept(true)일 경우 exception을 발생 시키지 않음을 보장한다. 
// 따라서, 아래 _Nothrow_compare 템플릿은 예외를 던질 지를 결정하는 expression이다. 

// NOTE: 아직 declval의 동작이 확연하게 이해되지 않는다. 
...
// _Ty_size는 Type Size의 약자로 해당 타잎의 크기를 뜻한다. 
// _Ty_size를 _Count만큼 얻을 때 바이트 크기. 
// NODISCARD는 [[nodiscard]] attribute 이다. 
...
// _INLINE_VAR는 C++ 17의 variable inline 기능이다. 초기값을 지정하는 static 멤버변수가 
// const static만 가능했는데 이를 개선하기위한 기능이다. 
// http://egloos.zum.com/sweeper/v/3213741
...
// trait는 해당 타잎의 특성을 표현한 것이다. ::operator new에 대한 wrapper이다. 
// __declspec은 microsoft c++의 키워드이다. https://docs.microsoft.com/en-us/cpp/cpp/allocator?view=msvc-170
// allocator 스펙은 Event Tracing for Windows에 이벤트를 발생 시킬 수 있다. 
...
// 2의 거듭제곱인 지를 계산한다. 
// 필요조건은 금방 확인할 수 있다. 충분조건인 지는 분류를 통해 확인할 수 있다. 
// 최상위 필드 외에 1인 경우가 있다면 적어도 최상위 필드는 1로 남기 때문에 
// 0이 될 수 없다. 
...
// enable_if_t는 MPL의 중요한 수단인 SFINAE 도구이다. 
// SFINAE는 Specialization Failure Is Not An Error의 약자로 특수화가 실패하면 
// 해당 템플릿 코드가 생성되지 않기 때문에 컴파일러 에러가 아니라는 뜻이다. 
// enable_if_t는 enable_if<true, Ty>을 사용하여 type에 접근할 수 있다. 
...
// variadic template는 template parameters와 parameter pack, unpacking으로 구성된다. 
// _Ty(Args...)으로 구성하는 함수이다. in-place new를 사용하고 addressof로 주소를 얻는다. 
// 
...
// _Guard_type이 필요한 이유는 무엇인가? :new에서 예외가 발생할 경우에도 
// 소멸자는 호출되므로 메모리를 해제하기 위해서 필요한 구조이다. 
// std::exchange는 
