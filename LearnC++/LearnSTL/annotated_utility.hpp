...
// std::initializer_list는 universal initializer를 지원하기 위한 타잎이다. 
// 컴파일러와 협업하여 코드 생성을 하므로 내부 동작의 실행가 상태 변경을 확인하기 어렵다. 
// cppreference 사이트의 설명이 좋다. 
template <class _Ty, class _Pr>
...
struct pair { // store a pair of values
    ...
    // explicit(expression)에서 expression이 true일 경우만 explicit가 적용된다.  
#if _HAS_CONDITIONAL_EXPLICIT
    template <class _Other1, class _Other2,
        enable_if_t<conjunction_v<is_constructible<_Ty1, _Other1>, is_constructible<_Ty2, _Other2>>, int> = 0>
    // 변환이 가능하면 암묵적인 변화을 사용하도록 허용하고, 그렇지 않은 경우는 변환을 허용하지 않는다. 
    ...
    // 타잎 추론이 가능하도록 하고 first와 second 필드를 갖는 타잎들을 지원하기 위해 
    // _Identity_t 간접 메타 클래스를 사용한다. 
